...
public class COMP_V2RED_AUTO_SAMPLE_Copy extends LinearOpMode {
    ...
    final double ARM_SCORE_SAMPLE_IN_LOW   = 92 * ARM_TICKS_PER_DEGREE;
    ...
    final double secondChomp = 19 * ARM_TICKS_PER_DEGREE;
    ...
    final double LIFT_SCORING_IN_HIGH_BASKET = 690 * LIFT_TICKS_PER_MM;
    final double LIMIT = 410 * LIFT_TICKS_PER_MM;
    final double firstSample = 395 * LIFT_TICKS_PER_MM;
    final double secondSample = 415 * LIFT_TICKS_PER_MM;
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()){
            ...
            armMotor(ARM_CLEAR_BARRIER);
            //brakePoint();
            currentPosition = myOtos.getPosition();
            timer.reset();
            while(opModeIsActive() && (Math.abs(currentPosition.h - -145) > 0.2) && timer.seconds() < 2){
                moveToTargetAngleUsingOTOS(-145);
            }
            stopMotion();
            //brakePoint();
            ...
            currentPosition = myOtos.getPosition();
            timer.reset();
            while(opModeIsActive() && (Math.abs(currentPosition.h - -122) > 0.2) && timer.seconds() < 2){
                moveToTargetAngleUsingOTOS(-122);
            }
            stopMotion();
            //brakePoint();
...
private void moveToTargetUsingOTOS(double targetX, double targetY) {
    ...
    while (opModeIsActive() && (Math.abs(currentPosition.x - targetX) > 0.1 || Math.abs(currentPosition.y - targetY) > 0.1 )) {
        double xPower = (targetX - currentPosition.x) * 0.1; // Proportional control
        double yPower = (targetY - currentPosition.y) * 0.1;
...
private void grabSample(double targetPosition){
    ...
    armMotor.setVelocity(2000);
    timer2.reset();
    while(Math.abs(liftMotor.getCurrentPosition()-targetPosition) > 20 && opModeIsActive() && timer2.seconds() < 3){
...
private void forward(double y, double speed){
    currentPosition = myOtos.getPosition();
    while(opModeIsActive() && currentPosition.y<y){
            fr.setPower(speed);
          fl.setPower(speed);
          bl.setPower(speed);
          br.setPower(speed);
...
private void backward(double y, double speed){
    currentPosition = myOtos.getPosition();
    while(opModeIsActive() && currentPosition.y>y){
            fr.setPower(-speed);
          fl.setPower(-speed);
