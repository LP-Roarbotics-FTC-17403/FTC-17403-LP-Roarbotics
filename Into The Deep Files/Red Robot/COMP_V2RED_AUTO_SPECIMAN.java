...
public class COMP_V2RED_AUTO_SPECIMAN extends LinearOpMode {
    ...
    final double ARM_UP = 1958 - 101;
    final double SPECIMAN_CLIP = 1390 - 101;
    ...
    final double SPECIMAN_EXTEND = 907;
    final double SPECIMAN_RETRACT = 443;
    ...
    public void runOpMode() {
        ...
        if (opModeIsActive()){
            ...
            moveToTargetUsingOTOS(-15,24);
            timer.reset();
            while(opModeIsActive() && (Math.abs(currentPosition.h - 0) > 0.2) && timer.seconds()<3){
            ...
            stopMotion();
            //brakePoint();
            armMotorOnly(1500,ARM_UP);
            //brakePoint();
            timer.reset();
             while(opModeIsActive() && liftMotor.getCurrentPosition() < SPECIMAN_EXTEND-0.1 && timer.seconds() < 1.75){
                liftMotor(SPECIMAN_EXTEND);
             }
            //brakePoint();
            armMotorOnly(1000, SPECIMAN_CLIP);
            //brakePoint();
            timer.reset();
             while(opModeIsActive() && liftMotor.getCurrentPosition() > SPECIMAN_RETRACT && timer.seconds() < 3){
                liftMotor(SPECIMAN_RETRACT);
             }
            //brakePoint();
            claw.setPosition(OPEN);
            sleep(100);
            while(opModeIsActive() && liftMotor.getCurrentPosition() > 0 && timer.seconds() < 3){
                liftMotor(0);
             }
            //brakePoint();
            moveToTargetUsingOTOS(5,5);
            //brakePoint();
            moveToTargetUsingOTOS(20,50);
            //brakePoint();
            moveToTargetUsingOTOS(33,50);
            //brakePoint();
            moveToTargetUsingOTOS(33,5);
            //brakePoint();
           // rotateMotors(0.4, "left",  0);
            
            
            moveToTargetUsingOTOS(33,10);
            timer.reset();
            while(opModeIsActive() && (Math.abs(currentPosition.h - 179) > 0.2) && timer.seconds()<3){
                moveToTargetAngleUsingOTOS(179);
                telemetry.addLine("HELP ME");
                telemetry.update();
            }
            timer.reset();
...
private void moveToTargetUsingOTOS(double targetX, double targetY) {
    ...
    while (opModeIsActive() && (Math.abs(currentPosition.x - targetX) > 0.1 || Math.abs(currentPosition.y - targetY) > 0.1 )) {
        ...
        xPower = Math.max(-0.5, Math.min(0.5, xPower));
        yPower = Math.max(-0.5, Math.min(0.5, yPower));
...
private void slowlyMoveToTargetUsingOTOS(double targetX, double targetY) {
    

    while (opModeIsActive() && (Math.abs(currentPosition.x - targetX) > 0.1 || Math.abs(currentPosition.y - targetY) > 0.1 )) {
        double xPower = (targetX - currentPosition.x) * 0.05; // Proportional control
        double yPower = (targetY - currentPosition.y) * 0.05;
        
        
        //Clamps the values between -1 and 1, see utility method below using Math.max and Math.min
        xPower = Math.max(-0.2, Math.min(0.2, xPower));
        yPower = Math.max(-0.2, Math.min(0.2, yPower));
