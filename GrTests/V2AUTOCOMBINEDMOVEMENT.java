...
public class COMP_R_AUTO2_SUB_Copy extends LinearOpMode {
    ...
    final double LIFT_SCORING_IN_HIGH_BASKET = 700 * LIFT_TICKS_PER_MM;
    final double LIMIT = 410 * LIFT_TICKS_PER_MM;
    final double firstSample = 390 * LIFT_TICKS_PER_MM;
    final double secondSample = 400 * LIFT_TICKS_PER_MM;
...
private void moveToTargetUsingOTOS(double targetX, double targetY) {
    ...
    if (opModeIsActive() && (Math.abs(currentPosition.x - targetX) > 0.1 || Math.abs(currentPosition.y - targetY) > 0.1 )) {
...
private void grabSample(double targetPosition){
    ...
    armMotor.setVelocity(2000);
    while(Math.abs(liftMotor.getCurrentPosition()-targetPosition) > 20 && opModeIsActive()){
...
private void liftMotor(double targetEncoder){
    if(Math.abs(liftMotor.getCurrentPosition()-targetEncoder)>1){
        double speed = Math.abs(liftMotor.getCurrentPosition()-targetEncoder) * 1.5; //value can be adjustable based on how fast you want the arm.
